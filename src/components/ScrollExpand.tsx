...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const renderMedia = () => {
    if (imgError || !src) {
      return (
        <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-[#130d2a] via-[#09090f] to-[#1c103f] flex items-center justify-center">
          <div
            aria-hidden="true"
            className="absolute inset-0 opacity-30 pointer-events-none"
            style={{
              backgroundImage: `radial-gradient(circle at 50% 50%, rgba(139,92,246,0.5) 0%, transparent 65%)`,
            }}
          />
          <div className="text-center p-6 relative z-0">
            <div className="w-20 h-20 sm:w-28 sm:h-28 mx-auto mb-3 rounded-full bg-gradient-to-tr from-purple-600 to-indigo-500 p-1 shadow-[0_0_40px_rgba(139,92,246,0.5)]">
              <div className="w-full h-full rounded-full bg-[#09090f] flex items-center justify-center">
                <span className="text-2xl sm:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-300">
                  MS
                </span>
              </div>
            </div>
            <p className="text-[10px] sm:text-xs font-mono uppercase tracking-widest text-purple-300/60">
              [Mohit Sharma Hero Photo]
            </p>
          </div>
        </div>
      );
