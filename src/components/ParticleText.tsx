...
export interface ParticleTextProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** Called once — right after the very first mount-gather animation finishes. */
  onComplete?: () => void;
}
...
const ParticleText = ({
  ...
  /** Ensures onComplete is called only once — on the initial mount gather. */
  ...
  useEffect(() => {
    ...
    const render = (now: number): void => {
      ...
      particles.forEach(particle => {
        ...
        if (gathering) {
          ...
          baseX +=
            Math.sin(driftTime * 0.9 + particle.seed * 10) * idleDrift * particle.depth;
          baseY +=
            Math.cos(driftTime * 0.75 + particle.depth * 10) * idleDrift * particle.depth;
        }
      ...
      if (gathering && complete) {
        ...
        // Fire onComplete only on the very first gather (initial mount).
    ...
    const sampleText = async (): Promise<void> => {
      ...
      if (maxLineWidth > maxTextWidth) {
        resolvedSize = Math.max(20, resolvedSize * (maxTextWidth / maxLineWidth));
      ...
      const singleLineH = resolvedSize * 1.15;
      let totalWidth = 0;
      lines.forEach(line => {
        const m = offCtx.measureText(line);
        const lLeft = Math.ceil(m.actualBoundingBoxLeft || 0);
        const lRight = Math.ceil(m.actualBoundingBoxRight || m.width);
        const lWidth = Math.max(1, lLeft + lRight);
        if (lWidth > totalWidth) totalWidth = lWidth;
      });

      const totalHeight = singleLineH * lines.length;
      const padding = Math.max(16, Math.ceil(resolvedSize * 0.1));

      offscreen.width = Math.ceil(totalWidth + padding * 2);
      ...
      lines.forEach((line, idx) => {
        const y = padding + (idx + 0.5) * singleLineH;
        offCtx.fillText(line, offscreen.width / 2, y);
      });
      ...
      const maxParticles = Math.max(1500, Math.min(12000, Math.floor((width * height) / 35)));
      ...
      if (reducedMotion) {
        ...
        // In reduced-motion mode the gather never animates, so fire onComplete immediately.
        if (!hasCompletedRef.current) {
          hasCompletedRef.current = true;
          onComplete?.();
        }
