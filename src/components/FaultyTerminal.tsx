...
float digit(vec2 p){
    ...
    float boxFillFactor = 0.0;
    if(uUseMouse > 0.5){
        vec2 mouseWorld = uMouse * uScale;
        float distToMouse = distance(s, mouseWorld);
        float mouseInfluence = exp(-distToMouse * 10.0) * uMouseStrength * 12.0;
        intensity += mouseInfluence;
        
        float ripple = sin(distToMouse * 20.0 - iTime * 5.0) * 0.1 * mouseInfluence;
        intensity += ripple;

        boxFillFactor = clamp(exp(-distToMouse * 8.0) * uMouseStrength * 3.0, 0.0, 1.0);
    }
    ...
    isOn = mix(isOn, 1.0, boxFillFactor);
    float brightness = isOn * (0.2 + y * 0.8) * (0.75 + x * 0.25);
    brightness = mix(brightness, 1.0, boxFillFactor * 0.85);
