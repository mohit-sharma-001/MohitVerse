...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (uLineStyle > 0.5) {
    ...
    if (uLineStyle < 1.5) {
      ...
      float dotMaskY2 = 1.0 - smoothstep(dotWidth2, dotWidth2 + fwidth(gridUV2.x * dotRepeat2), cy2);
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      const devOrient = win.DeviceOrientationEvent as unknown as { requestPermission?: () => Promise<void> } | undefined;
      if (enableGyro && devOrient && typeof devOrient.requestPermission === 'function') {
        try {
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      ...
      const bloomMat = bloomRef.current as unknown as { luminanceMaterial?: { threshold: number; smoothing: number } };
      if (bloomMat.luminanceMaterial) {
        bloomMat.luminanceMaterial.threshold = bloomThreshold;
        bloomMat.luminanceMaterial.smoothing = bloomSmoothing;
      }
    }
...
function smoothDampFloat(
  ...
  const rawChange = current - target;
  const originalTo = target;

  const maxChange = maxSpeed * smoothTime;
  const change = Math.sign(rawChange) * Math.min(Math.abs(rawChange), maxChange);
